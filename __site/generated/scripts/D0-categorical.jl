...
# This tutorial follows loosely [the docs](https://juliadata.github.io/CategoricalArrays.jl/latest/using.html).## ## Defining a categorical vector
...
# which, by  default, returns the labels in lexicographic order.## ## Working with categoricals## ### Ordered categoricals## You can specify that categories are *ordered* by specifying `ordered=true`, the order then follows that of the levels. If you wish to change that order, you  need to  use the `levels!` function.# Let's see two examples.
...
# ### Missing values## You can also have a categorical vector with missing values:
