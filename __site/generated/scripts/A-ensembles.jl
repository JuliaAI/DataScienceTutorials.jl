...
# where the `n=20` indicates how many models are present in the ensemble.## ### Training and testing an ensemble## Now that we've instantiated an ensemble, it can be trained and tested the same as any other model:
...
# Note that multiple measurements can be specified jointly. Here only on measurement is (implicitly) specified but we still have to select the corresponding results (whence the `[1]` for both  the `measurement` and `per_fold`).## ### Systematic tuning## Let's simultaneously tune the ensemble's `bagging_fraction` and the K-Nearest neighbour hyperparameter `K`. Since one of our models is  a field of the  other, we have nested hyperparameters:
...
# Note the `rng=42` seeds the random number generator for reproducibility of this example.## ### Reporting results## The best model can be accessed like so:
...
# \figalt{Hyperparameter heatmap}{A-ensembles-heatmap.svg}## Finally you can always just evaluate the model by reporting `rms` on the test set:
