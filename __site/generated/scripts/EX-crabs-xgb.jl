...
# which is pretty balanced. You could check the same on the test set and full set and the same comment would still hold.## ## XGBoost machine## Wrap a machine around an XGBoost model (XGB) and the data:
...
# \figalt{Cross entropy vs Num Round}{EX-crabs-xgb-curve1.svg}## So, in short, using more rounds helps. Let's arbitrarily fix it to 200.
xgb.num_round = 200;
...
# \figalt{Hyperparameter heatmap}{EX-crabs-xgb-heatmap.svg}## Let's extract the optimal model and inspect its parameters:
...
# ### More tuning (3)## Let's examine the effect of `subsample` and `colsample_bytree`:
...
# \figalt{Hyperparameter heatmap}{EX-crabs-xgb-heatmap2.svg}## Let's retrieve the best models:
xgb = fitted_params(mtm).best_model
@show xgb.subsample
