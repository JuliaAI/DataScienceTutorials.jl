...
# **Main author**: [Clarman Cruz](https://github.com/drcxcruz).## This juypter lab showcases MLJ in particular using the popular [GLM](https://github.com/JuliaStats/GLM.jl) Julia package. We are using two datasets.  One dataset was created manually for testing purposes.  The other data set is the CollegeDistance dataset from the [AER](https://cran.r-project.org/web/packages/AER/index.html) package in R.## We can quickly define our models in MLJ and study their results.  It is very easy and consistent.
...
# ## Reading the data## The CollegeDistance dataset was stored in a CSV file.  Here, we read the input file.
...
# ## Defining the Linear Model## Let see how many MLJ models handle our kind of target which is the y variable.
ms = models() do m
...
# ## Reading the Output of Fitting the Linear Model## We can quickly read the results of our models in MLJ.  Remember to compute the accuracy of the linear model.
...
# ## Reading the Output from the Prediction of the Logistic Model## The output of the MLJ model basically contain the same information as the R version of the model.
