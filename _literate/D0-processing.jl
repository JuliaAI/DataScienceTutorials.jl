...
using CSV
using MLJ: schema, std, mean, median
using MLJScientificTypes
#using MLJBase
using DataFrames#: DataFrame, describe, names, select, select!, Not, groupby, dropmissing, groupindices, rename!, aggregate
using UrlDownload
using PyPlot

# Import data

data = urldownload("https://github.com/tlienart/DataScienceTutorialsData.jl/blob/master/data/wri_global_power_plant_db_be_022020.csv?raw=true")
data = DataFrame(data)
...
for string in ["source", "generation"]
    colnames = names(data)
    select!(data, Not(filter(x -> occursin(string, String(x)), colnames)))
end
...
cap_mean = aggregate(capacity, [:country, :primary_fuel], mean)
cap_sum = aggregate(capacity, [:country, :primary_fuel], sum)
